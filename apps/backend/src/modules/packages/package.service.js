...
export const updatePackageService = async (id, body, files) => {
  ...
  if (isBestPackageBool && bestRank) {
    ...
    if (pkg.bestRank !== targetRank) {
      const existing = await packageRepository.findOne({ bestRank: targetRank, _id: { $ne: pkg._id } });
      if (existing) {
        existing.isBestPackage = false;
        existing.bestRank = null;
        await existing.save();
...
export const getPackageById = async (id) => {
  ...
  if (!currentPackage) {
    ...
    currentPackage = await packageRepository.findOne({ packageName: { $regex: slugRegex }, isDeleted: { $ne: true } });
  }
