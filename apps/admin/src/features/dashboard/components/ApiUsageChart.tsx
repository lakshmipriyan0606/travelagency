    ...
    return (
      <div className="bg-white/95 backdrop-blur-xl border border-neutral-200 p-4 rounded-xl shadow-xl shadow-neutral-200/50 min-w-[150px]">
        ...
        <p className="text-xs text-neutral-500 font-medium">
          Hits: <span className="text-green-600 font-bold">{count}</span>
        </p>
...
export function ApiUsageChart({ apiStats, selectedRoute, setSelectedRoute, apiRouteDetails, compact }: ApiUsageChartProps) {
  ...
  return (
    <PanelCard icon={Globe} iconClassName="text-green-500" title="Top API Routes">
      <div className="p-5">
        <div className={`w-full cursor-pointer ${compact ? "h-[250px]" : "h-[350px]"}`}>
          {apiStats.length > 0 ? (
            ...
            <EmptyState
              ...
              description="Production traffic will appear here."
            />
          )}
        </div>
      </div>
                    ...
                    >
                      <span
                        className={`text-sm font-bold ${
                          s.status < 400 ? "text-green-600" : s.status < 500 ? "text-amber-600" : "text-red-600"
                        }`}
                      >
                        HTTP {s.status}
                      </span>
