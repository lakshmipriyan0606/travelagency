          ...
          {(["upload", "url"] as const).map((source) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
                imageSource === source ? "bg-white text-primary shadow-sm" : "text-neutral-400 hover:text-neutral-600"
              }`}
            >
              {source}
            </button>
      ...
      {imageSource === "upload" ? (
        !imageUrl ? (
          ...
          <div className="relative group rounded-[32px] overflow-hidden shadow-2xl ring-1 ring-neutral-200">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
              <button type="button" onClick={() => setValue("url", "", { shouldValidate: true })} className="w-12 h-12 rounded-2xl bg-red-500 text-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform cursor-pointer">
                <X size={20} />
              </button>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <input {...register("url")} className="w-full px-6 py-4 rounded-2xl border border-neutral-200 focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all text-sm font-medium" placeholder="Paste external image URL here..." />
          </div>
        </div>
      )}
