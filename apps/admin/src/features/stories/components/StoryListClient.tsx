...
export default function StoryListClient() {
  ...
  const row1Stories = stories.filter((s: Story) => s.row === 1);
  const row2Stories = stories.filter((s: Story) => s.row === 2);

  const renderRowTable = (rowStories: Story[], title: string, rowNum: number) => (
      ...
      <div className="px-6 pb-2">
        <div className="grid grid-cols-1 gap-4">
          {rowStories.length === 0 ? (
            ...
            rowStories.map((story: any, index: number) => (
                ...
                <div className="flex gap-6">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-3">
                      <button onClick={() => moveMutation.mutate({ id: story._id, direction: "up" })} disabled={index === 0} className="p-2 rounded-xl hover:bg-neutral-50 text-neutral-400 hover:text-primary transition-all disabled:opacity-10 cursor-pointer shadow-sm border border-neutral-50 bg-white" title="Move Up">
                        <ArrowUp size={16} />
                      </button>
                      <button onClick={() => moveMutation.mutate({ id: story._id, direction: "down" })} disabled={index === rowStories.length - 1} className="p-2 rounded-xl hover:bg-neutral-50 text-neutral-400 hover:text-primary transition-all disabled:opacity-10 cursor-pointer shadow-sm border border-neutral-50 bg-white" title="Move Down">
                        <ArrowDown size={16} />
                      </button>
                      <div className="w-px h-5 bg-neutral-100 mx-2" />
                      <button onClick={() => handleEdit(story._id)} className="p-2 rounded-xl hover:bg-blue-50 text-neutral-400 hover:text-blue-500 transition-all cursor-pointer shadow-sm border border-neutral-50 bg-white" title="Edit">
                        <Pencil size={16} />
                      </button>
                      <button onClick={() => { if(window.confirm("Delete this customer story?")) deleteMutation.mutate(story._id); }} className="p-2 rounded-xl hover:bg-red-50 text-neutral-400 hover:text-red-500 transition-all cursor-pointer shadow-sm border border-neutral-50 bg-white" title="Delete">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
      ...
      <div className="bg-white rounded-[40px] shadow-2xl shadow-neutral-200/30 border border-neutral-100 overflow-hidden">
        <div className="p-8 sm:p-10 border-b border-neutral-100 flex flex-col sm:flex-row justify-between items-center bg-neutral-50/50 gap-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button onClick={() => normalizeMutation.mutate()} disabled={normalizeMutation.isPending} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white text-neutral-600 border border-neutral-200 px-6 py-4 rounded-[20px] text-xs font-black hover:bg-neutral-50 transition-all shadow-sm active:scale-95 cursor-pointer">
