...
export const EditorToolbar = ({ editor, onOpenPrompt }: { editor: any, onOpenPrompt: (config: any) => void }) => {
  ...
  const iconBtnClass =
    'p-2 rounded-md transition-colors text-white/50 hover:bg-white/[0.06] hover:text-white/80 disabled:opacity-40'

  return (
    <div className="flex flex-wrap items-center gap-1 p-2 border-b border-white/[0.08] bg-[var(--ent-elevated,#1c1c22)]/95 backdrop-blur-md sticky top-20 z-20 rounded-t-xl">
      ...
      <button type="button" onClick={addImage} className={`${iconBtnClass} flex items-center gap-1`} title="Add Image via Link"><LinkIcon size={18} className="text-[#F8B400]" /><span className="text-[10px] font-bold">IMG</span></button>
      <div className="w-px h-6 bg-white/15 mx-1"></div>
      <button type="button" onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().chain().focus().undo().run()} className={iconBtnClass} title="Undo"><Undo size={18} /></button>
      <button type="button" onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().chain().focus().redo().run()} className={iconBtnClass} title="Redo"><Redo size={18} /></button>
    </div>
  )
}
