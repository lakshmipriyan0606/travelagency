...
import { getDestinations, deleteDestination, moveDestination, normalizeDestinationsOrder } from "@/api/admin/destination.api";
...
export default function DestinationAdminList() {
                ...
                <div className="p-8 sm:p-10 border-b border-neutral-100 flex flex-col sm:flex-row justify-between items-center bg-neutral-50/50 gap-6">
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        {destinations.length < 4 ? (
                            <button
                                onClick={() => { setEditId?.(null); setActive?.("CreateDestination"); }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {destinations.length === 0 ? (
                            ...
                            destinations.map((dest: any, index: number) => (
                                <div key={dest._id} className="group relative bg-white border border-neutral-100 rounded-[32px] p-6 shadow-sm hover:shadow-2xl hover:shadow-neutral-200/50 transition-all duration-500 overflow-hidden">
                                     {/* Background visual number */}
                                     ...
                                     <div className="flex gap-6 relative z-10">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between py-1">
                                            ...
                                            <div className="flex items-center justify-end gap-1.5 mt-4">
                                                <button
                                                    onClick={() => moveMutation.mutate({ id: dest._id, direction: "up" })}
                                                    disabled={index === 0}
                                                ...
                                                <button
                                                    onClick={() => moveMutation.mutate({ id: dest._id, direction: "down" })}
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if(window.confirm("Delete this destination from elite 4?")) deleteMutation.mutate(dest._id);
                                                    }}
